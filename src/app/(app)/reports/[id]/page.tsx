...
export default async function ReportDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!data) notFound();
  const report = data as ReportWithAuthor;
              ...
              {report.foto_url && (
                <div>
                  ...
                  <img
                    ...
                    className="mt-2 max-h-96 w-auto max-w-full rounded-md border border-border"
                  />
                </div>
              )}

              {!isOwner && (
                <p className="flex items-start gap-2 rounded-md border border-border bg-accent/50 px-3 py-2.5 text-[12.5px] leading-relaxed">
