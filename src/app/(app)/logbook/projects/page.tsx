...
export const metadata = { title: "Proyek Konsultasi" };
...
export default async function ProjectsPage() {
  ...
  const projects = await fetchProjectsOverview(supabase, user.id);
          ...
          <Stagger className="grid gap-3 sm:grid-cols-2">
            {projects.map((p) => (
              <StaggerItem key={p.id}>
                <Link href={`/logbook/projects/${p.id}`} className="block h-full">
