...
/*
  ...
  peramban. Logika di dalamnya sama persis dengan rute yang digantikan; yang
  berubah hanya cara sesi dibaca (header Authorization, bukan cookie) dan kedua
  endpoint kini digabung lewat field `action` supaya cuma ada satu fungsi.
*/
...
Deno.serve(async (req) => {
  ...
  // pada rute lama. Service role sengaja TIDAK dipakai di sini.
  ...
  try {
    ...
    const { data: chunks, error: matchErr } = await supabase.rpc("match_branding_chunks", {
    ...
    return fail(`Generasi AI gagal: ${message}`, status);
  }
});
